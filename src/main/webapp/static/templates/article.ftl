...
<html lang="zh">
				...
				<div class="panel">
					...
					<div class="flex-box" id="center-left">
						<div class="wrapper majority article" id="article">
							<div class="info">
								<span><span b>分类: </span>视频教程</span><span><span b>浏览: </span><span id="viewCount" data-id="${id}">0</span></span>
							</div>
							<#--这是模板生成的地方-->
	...
	<script type="text/javascript">
		$(function (){
			let objId = $("#viewCount").data("id");
			//获取请求来刷新浏览次数
			//id
			$.ajax({
				type: "GET",
				url:"/addViews/addViews",
				data:{id: objId},
				success:function (data){
					if (data.code === 200) {
						document.getElementById("viewCount").innerText=data.data.count;
					}else{
						alert(data.msg);
					}
				}
